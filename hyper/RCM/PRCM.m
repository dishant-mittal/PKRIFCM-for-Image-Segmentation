...
for index=(C+1):datapoints
    index
    for j=1:C
        distance(index,j)=sum((Im(index,:)-transpose(c_means(:,j))).^2);
    ...
    [A,pos]= sort(distance(index,:));
    max1=pos(1);
    max2=pos(2);
    
    diff=distance(index,max1)-distance(index,max2);
    
    
    
    if (distance(index,max2)-distance(index,max1))<=0.000001
        flag=1;
        ...
        while(c_approx(2,max1,count2)~=-1 && count2<=datapoints)
            dist_sum=dist_sum+(distance(c_approx(2,max1,count2),max1))^2;
            count2=count2+1;
        end
        ...
        sum1;
        while(c_approx(2,max1,count2)~=-1 && count2<=datapoints)   
            sum2=sum2+double(Im(c_approx(2,max1,count2),:)).*double((1.0/(1.0+((distance(c_approx(2,max1,count2),max1))^2)/gamma))^2);
            count2=count2+1;
        end
        
        if ((count1-1)~=0 && (count2-1)~=0)
            c_means(:,max1)=((sum1.*w_low)./(double(count1-1))+((sum2.*w_up)./double(count2-1))); 
        end               
        if ((count1-1)~=0 && (count2-1)==0)
        ...
        while(c_approx(2,max2,count2)~=-1 && count2<=datapoints)
            dist_sum=dist_sum+(distance(c_approx(2,max2,count2),max2))^2;
            count2=count2+1;
        end
        gamma=dist_sum/double(count2-1);
        ...
        sum1;
        while(c_approx(2,max2,count2)~=-1 && count2<=datapoints)   
            sum2=sum2+double(Im(c_approx(2,max2,count2),:)).*double((1.0/(1.0+((distance(c_approx(2,max2,count2),max2))^2)/gamma))^2);
            count2=count2+1;
        end
       
        if ((count1-1)~=0 && (count2-1)~=0)
            c_means(:,max2)=((sum1.*w_low)./(double(count1-1))+((sum2.*w_up)./double(count2-1))); 
        end               
        if ((count1-1)~=0 && (count2-1)==0)
...
for i=1:C
    ...
    while(c_approx(2,i,count2)~=-1 && count2<=datapoints)
        dist_sum=dist_sum+(distance(c_approx(2,i,count2),i))^2;
        count2=count2+1;
    end
    gamma=dist_sum/double(count2-1);
    count1=1;
    ...
    while(c_approx(2,i,count2)~=-1 && count2<=datapoints)   
        sum2=sum2+double((distance(c_approx(2,i,count2),i))^2)*double((1.0/(1.0+((distance(c_approx(2,i,count2),i))^2)/gamma))^2);
        count2=count2+1;
    end
    
    if ((count1-1)~=0 && (count2-1)~=0)
        S(i)=((sum1*w_low)/(double(count1-1))+((sum2*w_up)/double(count2-1))); 
    end               
    if ((count1-1)~=0 && (count2-1)==0)
...
for i=1:C
    ...
    while((c_approx(1,i,count)~=-1 && count<=datapoints))  
        A(i)=A(i)+double((distance(c_approx(1,i,count),i))^2);
        count=count+1;
    end
...
for i=1:C
    ...
    while((c_approx(2,i,count)~=-1 && count<=datapoints))  
        B(i)=B(i)+double((distance(c_approx(2,i,count),i))^2);
        count=count+1;
    end
end
